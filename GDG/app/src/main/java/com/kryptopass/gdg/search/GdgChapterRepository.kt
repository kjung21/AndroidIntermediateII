...
import com.kryptopass.gdg.network.GdgApiService
import com.kryptopass.gdg.network.GdgChapter
import com.kryptopass.gdg.network.GdgResponse
import com.kryptopass.gdg.network.LatLong
...
class GdgChapterRepository(gdgApiService: GdgApiService) {
    ...
    /**
     * A single network request, the results won't change.
     * Do not add an offline cache, so result will be cached in memory.
     */
    ...
    private suspend fun sortedData(): SortedData = withContext(Dispatchers.Main) {
        // We need to ensure we're on Dispatchers.Main so that this is not running
        // on multiple Dispatchers and we modify the member inProgressSort.

        // Since this was called from viewModelScope, that will always be a simple if check (not expensive),
        // but by specifying the dispatcher we can protect against incorrect usage.

        // if there's currently a sort running (or completed) wait for it to complete and return that value otherwise,
        // start a new sort with no location (the user has likely not given us permission to use location yet)
        inProgressSort?.await() ?: doSortData()
    }

    /**
     * Call this to force a new sort to start.
     * This will start a new coroutine to perform the sort.
     * Future requests to sorted data can use the deferred in [inProgressSort]
     * to get the result of the last sort without sorting the data again.
     * This guards against multiple sorts being performed on the same data, which is inefficient.
     * This will always cancel if the location changes while the sort is in progress.
    ...
    private suspend fun doSortData(location: Location? = null): SortedData {
        ...
        // coroutineScope will automatically wait for anything started via async {} or await{} in it's block to complete.
     ...
     * Note, by convention this class won't sort on the Main thread.
     * This is not a public API and should only be called by [doSortData].
     */
